...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!couple.pixSetting?.enabled) return NextResponse.json({ error: "Pix não configurado" }, { status: 400 });
  ...
  if (gift.giftMode === GiftMode.UNIQUE) {
    ...
    if (blocked) return NextResponse.json({ error: "Presente reservado/indisponível" }, { status: 409 });
  }
